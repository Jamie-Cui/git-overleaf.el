...
updates only ~refs/git-overleaf/remote~, pull runs ~git-overleaf-pull~, and
push normally runs ~git-overleaf-push~.  The push action honors Magit's
...
elsewhere" action also routes a no-argument fetch through git-overleaf;
branches and refspecs are unsupported.  All other Git remotes retain
Magit's normal behavior.  Explicit =O= fetch bypasses the automatic remote
refresh throttle.
