...
~overleaf-project-read-cookies-from-file~ and
...
ordinary Magit refreshes do not repeatedly download snapshots.
...
| ~overleaf-project-cache-cookies~ | Reuse loaded cookies instead of reloading storage each time. |
...
| ~overleaf-project-curl-connect-timeout~ | Maximum time allowed for curl connection setup. |
| ~overleaf-project-curl-max-time~ | Maximum time allowed for non-download curl requests. |
| ~overleaf-project-curl-download-max-time~ | Maximum time allowed for project zip downloads; nil by default. |
| ~overleaf-project-curl-download-speed-limit~ | Low-speed threshold for project zip downloads. |
| ~overleaf-project-curl-download-speed-time~ | Seconds below the low-speed threshold before aborting a download. |
...
| ~overleaf-project-log-buffer-name~ | Global log buffer name. |
| ~overleaf-project-log-echo~ | Echo log entries in the minibuffer. |
| ~overleaf-project-log-time-format~ | Timestamp format for log entries. |
| ~overleaf-project-magit-auto-refresh-remote~ | Automatically refresh remote snapshots from Magit. |
| ~overleaf-project-magit-auto-refresh-remote-interval~ | Minimum seconds between automatic Magit remote refreshes. |
...
~*overleaf-project-log*~ buffer by default.  Each entry includes a
timestamp, log level, and the best available project, repository, or
Overleaf URL context, which helps distinguish logs from multiple
projects.
