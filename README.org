...
For a repository created by an older git-overleaf version, run
~git-overleaf-register-remote~ once.  If =overleaf= is already a real Git
remote, clone/init leaves it untouched and reports that an alternate name
must be registered.  Normal ~git remote rename~ and ~git remote remove~
work; a removed logical remote is not recreated automatically.
...
=O= action labelled with the logical remote name.  These actions are
branchless: fetch updates only ~refs/git-overleaf/remote~, pull runs
~git-overleaf-pull~, and push runs ~git-overleaf-push~.  Active Git-only
transient arguments are rejected.  Selecting the logical remote from
Magit's ordinary "fetch elsewhere" action also routes a no-argument fetch
through git-overleaf; branches and refspecs are unsupported.  All other
Git remotes retain Magit's normal behavior.  Explicit =O= fetch bypasses
the automatic refresh throttle.
