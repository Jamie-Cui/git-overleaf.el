...
~git-overleaf-fetch~, ~git-overleaf-overwrite-local~,
...
** Overwrite the local branch from Overleaf
...
This is the destructive counterpart of a forced push: it downloads the
latest Overleaf snapshot, creates a safety ref for the previous local
~HEAD~ when local backups are enabled, and replaces the current branch,
index, and working tree with the remote snapshot.  Staged, unstaged, and untracked changes are discarded;
ignored files are preserved.  The interactive command always asks for
confirmation and detached ~HEAD~ is rejected.

Only after the snapshot has downloaded successfully does the command
clean and reset the local repository.  On success it updates both the
base and remote refs and clears any pending Overleaf pull.  The safety ref
under ~refs/git-overleaf/backups/~ preserves committed local history, but
does not preserve discarded uncommitted or untracked content.
...
state is intended to replace the remote state.  The interactive command
always asks for confirmation.  This is git-overleaf's semantic equivalent
of ~git push --force~.

A successful overwrite also abandons and clears a pending Overleaf pull,
so this command is the recovery path after a pull was manually aborted or
its pending metadata became stale.  It never aborts a Git merge or discards
unresolved conflicts automatically: first resolve and commit the merge, or
run ~git merge --abort~, then run the overwrite again.  If the overwrite
fails, the pending state is retained so it can be retried safely.
...
To abandon the pull and keep the local version instead, resolve the
conflicts or run ~git merge --abort~, then run
~git-overleaf-overwrite-remote~.  This explicitly replaces Overleaf with
the resulting local ~HEAD~ and clears the pending pull only after a
successful overwrite.

To abandon the pull and accept the remote version instead, run
~git-overleaf-overwrite-local~.  After confirmation it discards the merge
worktree and local changes, replaces the branch from Overleaf, and clears
the pending pull only after the local overwrite succeeds.
...
Clone and init also register a branchless logical Git remote named
=overleaf= by default.  It has no URL, push URL, or refspec; its local
config marker associates it with the Overleaf project while
=skipFetchAll= and =skipDefaultUpdate= keep ordinary Git bulk operations
from contacting it.  The latest successfully observed snapshot is stored
in ~refs/git-overleaf/remote~, independently of the synchronized base in
~refs/git-overleaf/base~.  Normal remotes such as =origin= are unchanged.

For a repository created by an older git-overleaf version, either run
~git-overleaf-register-remote~ once or use an explicit uppercase =O=
operation in Magit, which offers to register the remote before continuing.
If =overleaf= is already a real Git remote, clone/init leaves it untouched;
interactive Magit registration prompts for an alternate name.  Normal
~git remote rename~ and ~git remote remove~ work.  Opening or refreshing a
Magit status buffer never registers a remote, so a removed logical remote is
not recreated without approval from an explicit =O= operation.
...
Push and overwrite operations report the completed content mutations and
...
=O= action labelled with the logical remote name.  When the remote is not
registered, the label instead says that the action will register it and
then fetch, pull, or push.  Selecting it asks for confirmation, registers
the default name (or prompts for another name after a collision), and
continues the requested operation.  These actions are branchless: fetch
normally updates only ~refs/git-overleaf/remote~, pull runs
~git-overleaf-pull~, and push normally runs ~git-overleaf-push~.  The fetch
action accepts =-f=/=--force= and, after confirmation, runs
~git-overleaf-overwrite-local~.  The push action honors Magit's
force arguments: =-f= (force with lease) uses the same guarded push because
git-overleaf already rejects a remote snapshot that differs from its
recorded base, while =-F= (force) asks for confirmation and runs
~git-overleaf-overwrite-remote~.  Other active Git-only transient arguments
are rejected.  Selecting the logical remote from Magit's ordinary "fetch
elsewhere" action also routes a fetch through git-overleaf and honors the
same force argument; branches and refspecs are unsupported.  All other Git
remotes retain Magit's normal behavior.  Explicit =O= fetch bypasses the
automatic remote refresh throttle.
...
| =F= | Overwrite local state from Overleaf after confirmation. |
| =g= | Refresh the remote snapshot. |
| =l= | Pull from Overleaf. |
| =p= | Push to Overleaf. |
| =r= | Register the logical Overleaf remote. |
| =O= | Force-push to the remote after confirmation. |
...
| ~git-overleaf-overwrite-local~ | Replace the current local branch and working tree from Overleaf. |
