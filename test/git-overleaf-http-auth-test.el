...
(ert-deftest git-overleaf-http-auth-test-project-list-parses-prefetched-blob ()
  (let* ((git-overleaf-url "https://example.overleaf.test")
         ...
         (headers nil))
    (cl-letf (((symbol-function 'git-overleaf--curl-request)
               (lambda (_method _url request-headers &optional _body)
                 (setq headers request-headers)
                 html)))
      (should (equal (git-overleaf-list)
                     '((:id "p1" :name "One"))))
      (should (member "Accept: application/json" headers)))
    (cl-letf (((symbol-function 'git-overleaf--curl-request)
               (lambda (&rest _args) "<html></html>")))
      (should-error (git-overleaf-list)
                    :type 'user-error))))

(ert-deftest git-overleaf-http-auth-test-project-list-guides-reauthentication ()
  (let ((git-overleaf-url "https://example.overleaf.test")
        (git-overleaf-cookies "sid=1")
        (git-overleaf--current-cookies nil))
    (cl-letf (((symbol-function 'git-overleaf--curl-request)
      ...
      (let ((err (should-error (git-overleaf-list) :type 'user-error)))
        (should (string-match-p "session may have expired"
                                (error-message-string err)))
        (should (string-match-p "git-overleaf-authenticate"
                                (error-message-string err)))))
    (cl-letf (((symbol-function 'git-overleaf--curl-request)
               (lambda (&rest _args)
                 "<html><form action=\"/login\"></form></html>")))
      (let ((err (should-error (git-overleaf-list) :type 'user-error)))
        (should (string-match-p "Log in to Overleaf again"
                                (error-message-string err)))))
    (cl-letf (((symbol-function 'git-overleaf--curl-request)
               (lambda (&rest _args) (error "network unavailable"))))
      (let ((err (should-error (git-overleaf-list) :type 'error)))
